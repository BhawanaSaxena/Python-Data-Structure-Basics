...
#the given data where key is the ticket number and value is the age of person
data = {
...
#Takig age as input from user
age= int(input("Enter age"))

#using two counters to add the ticket prices , initialized 0
org_price,new_price = 0,0

#using for loop to iterate over the values of dict. data
#calculating the price using if statement , if age less than 18 , 5$ otherwise 20$
for org_age in data.values():
    if org_age<18:
        org_price+=5
    else:
        org_price+=20

#using for loop to iterate over data.values() . If statement used with condition : if person's age less than user iput age ,5$ else 20$
for new_age in data.values():
    if new_age<age:
        new_price+=5
    else:
        new_price+=20

#total calculated to check how much money office can make
total = ((new_price-org_price/org_price)*100

#used abs if negative value comes in total .
print(int(abs(total)))
